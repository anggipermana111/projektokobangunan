...
public class DAO_Pengguna implements Service_Pengguna{
    ...
    public void perbaruiData(Model_Pengguna moduser) {
        PreparedStatement st = null;
        String sql = "UPDATE pengguna SET nama_pengguna=?, username=?, telp_pengguna=?, alamat_pengguna=?, level=? WHERE id_pengguna='"+moduser.getId_pengguna()+"'";
        try{
            st = connection.prepareStatement(sql);
            ...
            st.setString(3, moduser.getTelp_pengguna());
            st.setString(4, moduser.getAlamat_pengguna());
            st.setString(5, moduser.getLevel());
